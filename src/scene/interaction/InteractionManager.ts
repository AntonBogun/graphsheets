...
import { MouseSelectionHandler } from "./MouseSelectionHandler.js";
...
export class InteractionManager {
    ...
    //MARK: Event Listeners
        ...
        document.onmouseup = (event) => {
            for(const mouseHandler of this.mouseHandlers) {
                mouseHandler.processMouseUp(event, this.position);
            }
            // this.isDragging = false;
            // this.initialPosition = new Vec2d(0, 0);
            // this.lastPosition = new Vec2d(0, 0);
        ...
        document.addEventListener("wheel", (event) => {
                ...
                let mouse_position = new Vec2d(2*(event.clientX - (window.innerWidth / 2)) / window.innerWidth, 
                                                ...
                                                "normalized");
                let world_mouse_position: Vec2d<"world"> = State.currentCamera?.normalizedToWorld(mouse_position)!;
                let world_mouse_position3: Vec3d = new Vec3d(world_mouse_position.x, world_mouse_position.y, 0.0);
                // Vec4d.mmul(
                ...
                let delta = (event.deltaX + event.deltaY)/(event.ctrlKey?50:100);
