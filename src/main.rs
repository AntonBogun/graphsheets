...
use bevy_rendering as br;
 fn main(){
	...
	let node1 = ge::Node::new("node1".to_string(), (150.0,0.0), 10.0 ,Arc::new(Mutex::new(grid1)));
	graph.lock().unwrap().add_node(node1.clone());
	println!("1");
	let grid2 = ge::Grid{ id: 0, cells: vec![]};
	let node2 = ge::Node::new("node2".to_string(), (0.0,150.0), 10.0 ,Arc::new(Mutex::new(grid2)));
